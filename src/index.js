...
function Text(text, pos) {
  ...
  this.paste_text = function (text_copied) {
    if (this.is_text_selected()) {
      ...
      const text = this.properties[rtv.frame].t;
  ...
  this.change_text = function (text) {
    const changed = this.properties[rtv.frame].t != text;
  ...
  this.mouse_drag = function (evt) {
    ...
    if (rtv.presenting) {
      ...
      } else if (this.command == 'slide' && this.point_in_text_rect(rtv.mouse.start)) {
      ...
      // shift it
      const { p } = props;
      const offset = { x: rtv.mouse.grid.x - rtv.mouse.gridLast.x, y: rtv.mouse.grid.y - rtv.mouse.gridLast.y };
...
function Menu(pos) {
  ...
  this.buttons.push(new Button(`ver: ${VERSION}`, { x: 0, y: 0 }, ((b) => {
...
function draw_axes(ctx) {
  ...
  if (csys_style == '3d' || csys_style == 'flat') {
    ...
    if (csys_style == '3d') {
      ...
      const w = rtv.win_width * 2;
      const h = rtv.win_height * 2;
...
window.onload = function () {
  ...
  document.getElementById('gen_script').onclick = function (evt) {
    ...
    const t = new Text('', { x: 20, y: rtv.win_height * 2 - 60 });
